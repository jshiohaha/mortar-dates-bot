...
from src.constants.constants import MONDAY, BOT_NAME, GROUP_SIZE, CURRENT_DATES_COMMAND, HELP_COMMAND, NEW_DATES_COMMAND
from src.utils.groupings import generate_new_groups
from src.utils.general import groupings_to_str
...
def handle_new_dates_command(graph_client, grouping_client):
    most_recent_grouping = grouping_client.get_latest_grouping()
    now = datetime.now()
    if now.weekday() == MONDAY and (now-most_recent_grouping['date']).days >= 6:
        all_groupings = grouping_client.get_all_groupings()
        return generate_new_dates(all_groupings, graph_client, grouping_client)
...
def generate_new_dates(all_groupings, graph_client, grouping_client):
